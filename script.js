...
const INITIAL_FAMILIES = [
    {
        ...
        members: [
            { name: 'Pedro Silva Oliveira', relationship: 'Filho', birthDate: '2015-08-20', education: 'Ensino Fundamental', occupation: 'Estudante', income: 0 },
            { name: 'Ana Silva Oliveira', relationship: 'Filha', birthDate: '2018-03-12', education: 'Ensino Fundamental', occupation: 'Estudante', income: 0 }
        ]
    },
    {
        id: '2',
        ...
        members: [
            { name: 'Carla dos Santos', relationship: 'Esposa', birthDate: '1982-01-10', education: 'Ensino Médio', occupation: 'Costureira', income: 1200 }
        ]
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Controle dinâmico do campo de Tipo de Auxílio Governamental
    const receivesAidSelect = document.getElementById('receives-aid-select');
    const aidTypeWrapper = document.getElementById('aid-type-wrapper');
    const aidTypeInput = document.getElementById('aid-type-input');

    if (receivesAidSelect && aidTypeWrapper && aidTypeInput) {
        receivesAidSelect.addEventListener('change', () => {
            if (receivesAidSelect.value === 'Sim') {
                aidTypeWrapper.classList.remove('hidden');
            } else {
                aidTypeWrapper.classList.add('hidden');
                aidTypeInput.value = '';
            }
        });
    }
...
document.getElementById('add-member-btn').addEventListener('click', () => {
    ...
    const education = document.getElementById('member-education').value.trim();
    const occupation = document.getElementById('member-occupation').value.trim();
    const incomeVal = document.getElementById('member-income').value;

    if (!name || !relation || !birthDate || !education || !occupation || incomeVal === '') {
        alert('Data de nascimento, Grau de Instrução, Profissão e Renda são obrigatórios para o membro familiar!');
        return;
    }

    const income = parseFloat(incomeVal) || 0;

    currentRegistrationMembers.push({ 
        name, 
    ...
    document.getElementById('member-income').value = '0';
...
function renderMembersInForm() {
    ...
    body.innerHTML = currentRegistrationMembers.map((m, index) => `
        <tr class="border-t border-slate-100">
            ...
            <td class="px-4 py-3 text-slate-500 text-xs">${m.relationship}</td>
            <td class="px-4 py-3 text-slate-500 text-xs">${m.birthDate ? m.birthDate.split('-').reverse().join('/') : '---'}</td>
            <td class="px-4 py-3 text-slate-500 text-xs">${m.education || '---'}</td>
            <td class="px-4 py-3 text-slate-500 text-xs">${m.occupation || '---'}</td>
            <td class="px-4 py-3 text-slate-700 text-xs font-semibold">R$ ${(m.income || 0).toFixed(2)}</td>
            <td class="px-4 py-3 text-right">
                <button type="button" onclick="removeMemberFromForm(${index})" class="text-rose-500 hover:text-rose-700 font-bold text-[10px] uppercase transition-colors">Remover</button>
            </td>
        </tr>
    `).join('');
}
...
document.getElementById('save-family-btn').addEventListener('click', () => {
    ...
    const newFamily = {
        ...
        receivesGovernmentAid: formData.get('receivesGovernmentAid'),
        governmentAidType: formData.get('receivesGovernmentAid') === 'Sim' ? formData.get('governmentAidType') : '',
...
window.viewFamily = (id) => {
    ...
    detailsSection.innerHTML = `
        <div class="flex items-center justify-between print:hidden mb-6">
            <div class="flex items-center gap-4">
                <button onclick="document.querySelector('.nav-item[data-section=\\'list\\']').click()" class="w-10 h-10 rounded-xl hover:bg-white flex items-center justify-center transition-colors">
                    <i data-lucide="chevron-left"></i>
                </button>
                <h2 class="text-2xl font-bold">Ficha de Acompanhamento</h2>
            </div>
            <div class="flex gap-2">
                <button onclick="window.print()" class="h-10 px-6 rounded-xl border border-slate-200 font-bold text-xs uppercase flex items-center gap-2 hover:bg-slate-50 transition-all"><i data-lucide="printer" class="w-4 h-4"></i> Imprimir Ficha</button>
                <button class="btn-primary h-10 px-6 rounded-xl text-xs uppercase flex items-center gap-2 shadow-lg shadow-blue-600/10"><i data-lucide="edit" class="w-4 h-4"></i> Editar Dados</button>
            </div>
        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bento-card p-5 space-y-4">
                    ...
                    <div class="flex gap-3 text-sm border-t pt-4">
                        ...
                        <span class="text-slate-600 leading-snug">${family.address}, ${family.neighborhood}${family.municipality ? ' - ' + family.municipality : ''}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="bento-card p-6">
                    ...
                    <div class="grid grid-cols-2 gap-y-6">
                        ...
                        <div class="col-span-2 border-t pt-4 mt-2">
                            <p class="text-[9px] uppercase font-bold text-slate-400">Auxílio Governamental</p>
                            <p class="text-sm font-semibold text-slate-700">${family.receivesGovernmentAid === 'Sim' ? (family.governmentAidType ? 'Sim (' + family.governmentAidType + ')' : 'Sim') : (family.receivesGovernmentAid === 'Não' ? 'Não recebe' : '---')}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left text-sm">
                        <thead>
                            <tr class="text-slate-400 border-b">
                                ...
                                <th class="pb-3 text-[10px] uppercase">Escolaridade</th>
                                <th class="pb-3 text-[10px] uppercase">Profissão</th>
                                <th class="pb-3 text-right text-[10px] uppercase">Renda</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${family.members.length > 0 ? family.members.map(m => `
                                <tr class="border-b border-slate-50">
                                    ...
                                    <td class="py-3 text-slate-500">${m.birthDate ? m.birthDate.split('-').reverse().join('/') : '---'}</td>
                                    <td class="py-3 text-slate-500">${m.education || '---'}</td>
                                    <td class="py-3 text-slate-500">${m.occupation || '---'}</td>
                                    <td class="py-3 text-right text-slate-700 font-semibold font-mono">R$ ${(m.income || 0).toFixed(2)}</td>
                                </tr>
                            `).join('') : '<tr><td colspan="6" class="py-4 text-center text-slate-400 italic font-medium">Nenhum membro registrado.</td></tr>'}
                        </tbody>
                    </table>
                </div>
        ...
        <div class="mt-12 pt-8 border-t border-slate-100">
            <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-10 text-center">Área de Assinaturas e Validação</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-16 pb-12">
                <div class="text-center">
                    <div class="signature-line mb-3"></div>
                    <p class="text-xs font-bold uppercase text-slate-600 tracking-widest">Assinatura do Responsável Familiar</p>
                    <p class="text-[9px] text-slate-400 mt-1 uppercase italic">(Documento de Identificação em mãos)</p>
                </div>
                <div class="text-center">
                    <div class="signature-line mb-3"></div>
                    <p class="text-xs font-bold uppercase text-slate-600 tracking-widest">Responsável Projeto Cativeiro</p>
                    <p class="text-[9px] text-slate-400 mt-1 uppercase italic">(Carimbo e Assinatura da ONG)</p>
                </div>
            </div>
        </div>

        <div class="text-center text-[10px] text-slate-400 uppercase tracking-widest font-bold pt-8 border-t mt-4 hidden print:block">
            Documento Oficial • Gerado em ${new Date().toLocaleString('pt-BR')} • Projeto Social Cativeiro
        </div>
    `;
